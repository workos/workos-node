...
import { serializeBatchCheckOptions, serializeCheckManyOptions, serializeCheckOptions } from "./serializers/check-warrant-options.serializer";
...
export class FGA {
  ...
  async check(checkOptions: CheckOptions, options: CheckRequestOptions = {}): Promise<CheckResult> {
    const { data } = await this.workos.post<CheckResultInterface>(`/fga/v1/check`, serializeCheckOptions(checkOptions), options);
    return new CheckResult(data);
  }

  async checkMany(checkOptions: CheckManyOptions, options: CheckRequestOptions = {}): Promise<CheckResult> {
    const { data } = await this.workos.post<CheckResultInterface>(`/fga/v1/check`, serializeCheckManyOptions(checkOptions), options);
    return new CheckResult(data);
  }

  async batchCheck(checkOptions: BatchCheckOptions, options: CheckRequestOptions = {}): Promise<CheckResult[]> {
    const { data } = await this.workos.post<CheckResultInterface[]>(`/fga/v1/check`, serializeBatchCheckOptions(checkOptions), options);
    return data.map((checkResult: CheckResultInterface) => new CheckResult(checkResult));
  }
  ...
  async getObject(object: WarrantObject | WarrantObjectLiteral): Promise<BaseWarrantObject> {
  ...
  async listObjects(options?: ListObjectOptions): Promise<AutoPaginatable<BaseWarrantObject>> {
  ...
  async updateObject(options: UpdateObjectOptions): Promise<BaseWarrantObject> {
    const objectType = isWarrantObject(options.object) ? options.object.getObjectType() : options.object.objectType;
    const objectId = isWarrantObject(options.object) ? options.object.getObjectId() : options.object.objectId;
  ...
  async batchWriteWarrants(options: WriteWarrantOptions[]): Promise<WarrantToken> {
  ...
  async listWarrants(options?: ListWarrantsOptions, requestOptions?: ListWarrantsRequestsOptions): Promise<AutoPaginatable<Warrant>> {
    return new AutoPaginatable(
  ...
  async query(options: QueryOptions, requestOptions: QueryRequestOptions = {}): Promise<AutoPaginatable<QueryResult>> {
    return new AutoPaginatable(
