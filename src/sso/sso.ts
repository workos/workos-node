...
import type { ListConnectionsOptions } from './interfaces/list-connections-options.interface';
import type { GetLogoutUrlOptions } from './interfaces/get-logout-url-options.interface';
import type { AuthorizeLogoutOptions } from './interfaces/authorize-logout-options.interface';
import type {
...
const serializeListConnectionsOptions = (
  ...
  if (options.connectionType !== undefined)
    wire.connection_type = options.connectionType;
  if (options.domain !== undefined) wire.domain = options.domain;
...
export class SSO {
  ...
  async listConnections(
    options?: ListConnectionsOptions,
  ): Promise<AutoPaginatable<Connection, PaginationOptions>> {
   ...
   */
  getLogoutUrl(options: GetLogoutUrlOptions): string {
    const query = toQueryString({ token: options.token });
    return `${this.workos.baseURL}/sso/logout?${query}`;
  }
  ...
  async authorizeLogout(
    options: AuthorizeLogoutOptions,
  ): Promise<SSOLogoutAuthorizeResponse> {
    const payload = options;
    const { data } = await this.workos.post<
      SSOLogoutAuthorizeResponseWire,
      SSOLogoutAuthorizeRequestResponse
    >('/sso/logout/authorize', serializeSSOLogoutAuthorizeRequest(payload));
    return deserializeSSOLogoutAuthorizeResponse(data);
  }
