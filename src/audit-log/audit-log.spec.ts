...
describe('AuditLog', () => {
  describe('publish', () => {
    it('posts Event successfuly', async () => {
      ...
      await expect(workos.AuditLog.publish(event)).resolves.toBeUndefined();
    });
    ...
    it('throws an UnauthorizedException when the api returns a 401', async () => {
      ...
      await expect(workos.AuditLog.publish(event)).rejects.toStrictEqual(
